"""INCY subscription link encryption (incy://crypt1/<payload>).

Pure-Python port of @incy/link-encoder@1.0.1 (Node.js). AES-256-GCM with a
key derived from KEYMAT_A/KEYMAT_B + fixed salts. Key fingerprint is
verified on first use against the value published in the npm package — if
INCY rotates to crypt2/ this module will refuse to silently produce
invalid links.

The encryption is obfuscation for UX, not cryptography for secrecy: the
key lives inside every INCY client.
"""

from __future__ import annotations

import base64
import hashlib
import json
import os
from typing import Optional

from cryptography.hazmat.primitives.ciphers.aead import AESGCM


_SALT_P1 = b'incy'
_SALT_P2 = b'deep'
_SALT_P3 = b'crypt1'
_SALT_P4 = b'v2026.06'
_KEYMAT_A_OFFSET = 1024
_KEYMAT_B_OFFSET = 2048
_KEYMAT_LEN = 32
_EXPECTED_FINGERPRINT = 'b6bf708471cc90043232967660aade86a50b4e57929db2e53c5fa34db624c08c'
_SCHEME = 'incy'
_HOST = 'crypt1'
_NAME_MAX_LEN = 128

_KEYMAT_A_B64 = ''
_KEYMAT_B_B64 = ''

_key_cache: Optional[bytes] = None


def _derive_key() -> bytes:
    global _key_cache
    if _key_cache is not None:
        return _key_cache

    a = base64.b64decode(_KEYMAT_A_B64)
    b = base64.b64decode(_KEYMAT_B_B64)
    if len(a) < _KEYMAT_A_OFFSET + _KEYMAT_LEN or len(b) < _KEYMAT_B_OFFSET + _KEYMAT_LEN:
        raise RuntimeError('incy_crypto: keymat assets are smaller than expected')

    km_a = a[_KEYMAT_A_OFFSET : _KEYMAT_A_OFFSET + _KEYMAT_LEN]
    km_b = b[_KEYMAT_B_OFFSET : _KEYMAT_B_OFFSET + _KEYMAT_LEN]
    seed = _SALT_P1 + _SALT_P2 + _SALT_P3 + _SALT_P4 + km_a + km_b
    k = hashlib.sha256(seed).digest()
    fp = hashlib.sha256(k).hexdigest()
    if fp != _EXPECTED_FINGERPRINT:
        raise RuntimeError(
            f'incy_crypto: derived K1 fingerprint mismatch (expected {_EXPECTED_FINGERPRINT}, got {fp}) '
            f'— keymat is out of sync with the published clients.'
        )

    _key_cache = k
    return k


def _sorted_compact_json(payload: dict) -> str:
    keys = sorted(payload.keys())
    parts = [
        json.dumps(k, ensure_ascii=False, separators=(',', ':'))
        + ':'
        + json.dumps(payload[k], ensure_ascii=False, separators=(',', ':'))
        for k in keys
    ]
    return '{' + ','.join(parts) + '}'


def _b64url_encode(data: bytes) -> str:
    return base64.urlsafe_b64encode(data).decode('ascii').rstrip('=')


def encrypt_incy_link(url: str, name: Optional[str] = None) -> str:
    """Encrypt a subscription URL into incy://crypt1/<payload>."""
    if not url or not isinstance(url, str):
        raise ValueError('encrypt_incy_link: url must be a non-empty string')

    key = _derive_key()
    payload: dict = {'url': url, 'v': 1}
    if name:
        payload['n'] = name[:_NAME_MAX_LEN]

    plaintext = _sorted_compact_json(payload).encode('utf-8')
    iv = os.urandom(12)
    ct_and_tag = AESGCM(key).encrypt(iv, plaintext, None)
    wire = iv + ct_and_tag
    return f'{_SCHEME}://{_HOST}/{_b64url_encode(wire)}'


SCHEME_VERSION = 'crypt1'
KEY_FINGERPRINT = _EXPECTED_FINGERPRINT
